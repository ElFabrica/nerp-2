...
- Garantir invalidação correta das queries (`kitchen.list` e `kitchen.columns.list`) após
  `kitchen.move` e após o CRUD de colunas (Etapa 3).
- Conferir responsividade da `/cozinha` (N colunas dinâmicas: scroll horizontal / empilham no
  mobile).
...
3. Org nova já mostra as **3 colunas padrão**; cadastrar "Mesa 18 - Batata Recheada" → entra na
   coluna de entrada (**Em Preparo**), tempo subindo.
4. Passar do tempo estimado → card âmbar → vermelho (atrasado).
5. **Arrastar** o card de Em Preparo → **Prontos** (ou botão `[→ Prontos]`).
...
7. **Arrastar** de Prontos → **Entregues** (ou `[→ Entregues]`) → some da TV; aparece na coluna
   **Entregues** e some dela após `AUTO_HIDE_MS`.
8. **Personalizar colunas** ("Gerenciar colunas"): renomear/recolorir/reordenar, definir limite
   WIP, esconder e adicionar uma coluna nova → o board reflete na hora.
9. Isolamento multi-tenant entre orgs diferentes.
